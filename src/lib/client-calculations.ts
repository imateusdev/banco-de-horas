...
export const clientTimeUtils = {
  ...
  getDailyStats: async (date: string, userId?: string): Promise<DailyStats> => {
    const records = userId ? await clientStorageUtils.getUserTimeRecords(userId) : await clientStorageUtils.getTimeRecords();
  ...
  getMonthlyStats: async (month: string, userId?: string): Promise<MonthlyStats> => {
    ...
    const goal = userId ? await clientStorageUtils.getUserMonthlyGoal(userId, month) : await clientStorageUtils.getMonthlyGoal(month);
  ...
  isValidTimeRange: (startTime: string, endTime: string): boolean => {
    if (!clientTimeUtils.isValidTimeFormat(startTime) || !clientTimeUtils.isValidTimeFormat(endTime)) {
      return false;
    }
  ...
  // Calcula horas extras acumuladas de todos os meses
  getAccumulatedExtraHours: async (userId?: string): Promise<import('@/types').AccumulatedHours> => {
    try {
      const records = userId ? await clientStorageUtils.getUserTimeRecords(userId) : await clientStorageUtils.getTimeRecords();
      const conversions = userId ? await clientStorageUtils.getUserHourConversions(userId) : await clientStorageUtils.getHourConversions();
      
      // Agrupa registros por mês
      const monthlyData: { [month: string]: { totalHours: number, goal: number } } = {};
      
      // Processa registros de trabalho e folga por mês
      for (const record of records) {
        ...
        // Trabalho adiciona, folga subtrai
        if (record.type === 'work') {
      ...
      // Calcula total de horas extras (apenas meses que excederam a meta)
